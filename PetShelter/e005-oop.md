...
- [X] `Animal` base class with core attributes and methods
- [X] `Dog` and `Cat` classes inherit from `Animal`
- [X] `Puppy`, `ServiceDog`, and `Kitten` properly extend their parents
- [X] `Shelter` class manages collection of animals
- [X] `speak()` method demonstrates polymorphism
- [X] All classes have proper `__str__` methods
- [X] `main()` demonstrates all features
