...
SELECT EXTRACT(YEAR FROM invoice_date) AS year, COUNT(*), SUM(total) FROM invoice GROUP BY year ORDER BY year;

-- how many line items were there for invoice #37
SELECT COUNT(*) FROM invoice WHERE invoice_id = 37;
...
SELECT artist.name, album.title FROM artist INNER JOIN album ON artist.artist_id = album.artist_id;

-- (inner keyword is optional for inner join)
-- All songs of the rock genre
SELECT track.name FROM track INNER JOIN genre ON track.genre_id = genre.genre_id WHERE genre.name = 'Rock';

-- Show all invoices of customers from brazil (mailing address not billing)
SELECT * FROM invoice INNER JOIN customer ON invoice.customer_id = customer.customer_id WHERE customer.country = 'Brazil';

-- Show all invoices together with the name of the sales agent for each one
SELECT * FROM invoice JOIN customer ON customer.support_rep_id = employee.employee_id JOIN employee ON invoice.customer_id = customer.customer_id;

-- Which sales agent made the most sales in 2009?


-- How many customers are assigned to each sales agent?
SELECT employee.first_name, COUNT(customer.support_rep_id) FROM employee JOIN customer ON employee.employee_id = customer.support_rep_id GROUP BY employee.first_name ORDER BY COUNT(customer.support_rep_id);

-- Which track was purchased the most in 2010?
SELECT track.name, COUNT(invoice_line.track_id) FROM track JOIN invoice_line ON track.track_id = invoice_line.track_id WHERE trackGROUP BY track.name ORDER BY COUNT(invoice_line.track_id) DESC;
